...
@pytest.mark.skip(reason=STUB_REASON)
...
def test_cite_span_retrieval(case: RetrievalCase):
    ...
    raise NotImplementedError(STUB_REASON)
...
KEYVAL_CASES: List[RetrievalCase] = [
    ...
    RetrievalCase(
        name='value_truncated_at_budget',
        record=[('k', 'w1 w2 w3 w4 w5 w6 w7 w8 w9 w10')],
        query='key=k; max_len tiny',
        expected='w1 w2',
        grounded=True,
        notes='CORNER: only one pair fits and the value is trimmed to budget; '
              'the (trimmed) value still appears verbatim in the record.',
    ),
    RetrievalCase(
...
@pytest.mark.skip(reason=STUB_REASON)
@pytest.mark.parametrize('case', GROUNDED_CASES, ids=[c.name for c in GROUNDED_CASES])
def test_grounded_target_is_substring_of_record(case: RetrievalCase):
    """Invariant 1 (needle-in-context): for every grounded case the serialized
    ``expected`` target must appear verbatim inside the serialized record —
    INCLUDING the composite cases, which is precisely where a fixed
    json.dumps / naive join would violate the invariant."""
    raise NotImplementedError(STUB_REASON)


@pytest.mark.skip(reason=STUB_REASON)
@pytest.mark.parametrize('case', COMPOSITE_CASES, ids=[c.name for c in COMPOSITE_CASES])
def test_composite_serialization_is_deterministic(case: RetrievalCase):
    ...
    (object / array / subtree / row(s)) is a pure function of the value and the
    record's serialization policy — repeated builds with the same seed and
    record yield byte-identical targets."""
    raise NotImplementedError(STUB_REASON)
...
def test_composite_serialization_follows_record_separator_policy(case: RetrievalCase):
    ...
    (compact vs spaced for JSON; outer-XML for subtrees; defined joins for
    SQL rows) so the composite target stays grounded. This pins that policy."""
    raise NotImplementedError(STUB_REASON)
